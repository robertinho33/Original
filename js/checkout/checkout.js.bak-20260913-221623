...
function renderTotals() {
    ...
    if (shipping) {
        shipping.textContent = getShippingCost() === 0 ? 'Grátis' : formatCurrency(getShippingCost());
    }
    if (total) {
...
function validateForm() {
    ...
    if (phoneDigits.length < 10) {
        setFieldError('customerPhone', 'Informe um telefone válido.');
        valid = false;
    }
    ...
    if (paymentMethod === 'pix') {
        if (!email?.value.trim()) {
            ...
            setFieldError('customerEmail', 'Informe um e-mail válido.');
            valid = false;
        }
    ...
    if (getDeliveryMethod() === 'delivery') {
        ...
        if (cepDigits && cepDigits.length !== 8) {
            setFieldError('addressZip', 'Informe um CEP válido.');
            valid = false;
        }
    }
...
if (
    ...
    try {
        ...
        if (message) {
            message.textContent =
                'Pedido recebido, mas não foi possível gerar o PIX. Tente novamente.';
        }
    }
}
...
async function handleSubmit(event) {
    ...
    if (!cart.length) {
        if (message) {
            ...
            message.textContent = 'Sua sacola está vazia. Volte à loja e adicione produtos.';
        }
        return;
    }
    ...
    try {
        ...
        if (message) {
            message.hidden = false;
            message.textContent = error?.message || 'Não foi possível finalizar o pedido. Tente novamente.';
        }
    } finally {
...
async function init() {
    try {
        ...
        if (message) {
            message.hidden = false;
            message.textContent = 'Não foi possível carregar o checkout. Atualize a página e tente novamente.';
        }
    }
}
