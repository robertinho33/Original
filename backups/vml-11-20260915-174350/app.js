...
function renderFilters() {
    ...
    filtersEl.innerHTML = `
        <button class="active" type="button" data-category="todos">
            Todos
        </button>
...
function getFilteredProducts() {
    ...
    if (currentCategory !== 'todos') {
        const categoryNorm = currentCategory.trim().toLowerCase();
        result = result.filter(product =>
            String(product.category).trim().toLowerCase() === categoryNorm
        );
    }

    // 2. Filtro por Busca (Nome/Descrição)
    if (currentSearchQuery.trim() !== '') {
        const queryNorm = currentSearchQuery.trim().toLowerCase();
        result = result.filter(product =>
            product.name.toLowerCase().includes(queryNorm) ||
            (product.description && product.description.toLowerCase().includes(queryNorm))
        );
    }
...
function renderPagination(totalItems) {
    ...
    const pages = [];
    const maxVisiblePages = 5; // Limite de botões visíveis
...
function renderProducts() {
    ...
    // Ajustar página caso os filtros reduzam o total de páginas
    const totalPages = Math.ceil(totalItems / ITEMS_PER_PAGE);
    ...
    productsEl.innerHTML = paginatedProducts.map(product => {
        const image = product.image
            ? `<img class="product-image" src="${escapeAttribute(product.image)}" alt="${escapeAttribute(product.name)}" loading="lazy">`
            : `<div class="product-image-placeholder">AURÉA</div>`;

        const weight = product.weight ? escapeHTML(product.weight) : '';

        return `
            <article class="product" data-product-sku="${escapeAttribute(product.sku)}" tabindex="0" role="button" aria-label="Ver detalhes de ${escapeAttribute(product.name)}">
                ...
                <div class="product-info">
                    ...
                    ${weight ? `<small class="product-weight">${weight}</small>` : ''}
                    <div class="product-bottom">
                        <span class="price">${formatCurrency(product.price)}</span>
                        <button class="add" type="button" data-add="${escapeAttribute(product.sku)}" aria-label="Adicionar à sacola">+</button>
                    </div>
                </div>
            </article>
        `;
